...
// --- catalog iPXE menu (netboot.xyz-style direct boot) ---------------
//
// The same catalog that powers the UI's Browse-catalog modal can be
// served as an iPXE menu. Any iPXE client (LAN PXE, the bootstrap
// stick, or anyone running ipxe binary directly) can chainload
// /catalog/menu.ipxe and get an interactive list to pick from. No
// auth code, no DB lookup, no per-machine record — just netboot. This
// is the "I have a fresh box and just need to install Ubuntu" path,
// complementing (not replacing) the tracked /api/v1/deployments flow.

func (h *handlers) catalogMenuIPXE(w http.ResponseWriter, r *http.Request) {
	c, err := catalog.Load()
	if err != nil {
	...
	b.WriteString("#!ipxe\n")
	b.WriteString("# Generated by deployserver — netboot.xyz-style menu of catalog entries.\n")
	b.WriteString("# Source: " + h.publicURL + "/catalog/menu.ipxe (catalog version " + c.Version + ")\n\n")

	b.WriteString(":start\n")
	b.WriteString("menu deployserver — pick a distro to net-install\n")
	for _, cat := range c.Categories {
		b.WriteString("item --gap -- ── " + cat.Name + " ──\n")
		for _, e := range cat.Entries {
			label := e.Name
			if len(label) > 60 {
				label = label[:57] + "..."
			}
			b.WriteString("item " + e.ID + " " + label + "\n")
		}
	}
	b.WriteString("item --gap\n")
	if c.Fallback != nil && c.Fallback.ChainURL != "" {
		b.WriteString("item netboot-xyz-fallback ▸ " + c.Fallback.Label + "\n")
	}
	b.WriteString("item exit Exit to iPXE shell\n")
	b.WriteString("choose --timeout 30000 --default exit selected || goto cancelled\n")
	...
	for _, cat := range c.Categories {
		for _, e := range cat.Entries {
			kernel, _ := e.Media["kernel_url"].(string)
			initrd, _ := e.Media["initrd_url"].(string)
			args, _ := e.Media["kernel_args"].(string)
			chain, _ := e.Media["chain_url"].(string)
			b.WriteString(":" + e.ID + "\n")
			if chain != "" {
				// Menu-only entry; delegate to upstream iPXE script
				// (typically netboot.xyz). The chained menu handles
				// its own kernel/initrd selection.
				b.WriteString("echo Loading " + e.Name + " via " + chain + " ...\n")
				b.WriteString("chain " + chain + " || goto failed\n\n")
				continue
			}
			if kernel == "" || initrd == "" {
				b.WriteString("echo This catalog entry (" + e.ID + ") is a stub — no kernel/initrd URL configured.\n")
				b.WriteString("echo Use the operator UI to register custom URLs, or pick a different entry.\n")
	...
	// Fallback chain to upstream catalog (e.g. netboot.xyz) for any
	// distro not in our curated set.
	if c.Fallback != nil && c.Fallback.ChainURL != "" {
		b.WriteString(":netboot-xyz-fallback\n")
		b.WriteString("echo Loading netboot.xyz menu ...\n")
		b.WriteString("chain " + c.Fallback.ChainURL + " || goto failed\n\n")
	}

	b.WriteString(":failed\n")
	b.WriteString("echo Boot failed. Sleeping 5s before returning to menu.\n")
